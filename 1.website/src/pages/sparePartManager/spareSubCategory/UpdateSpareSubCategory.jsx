...
export default function UpdateSpareSubCategory({
  open,
  handleOk,
  handleCancel,
  spareCategories,
  ...
  useEffect(() => {
    form.setFieldsValue(spareSubCategory);
  }, [spareSubCategory, form]);

  const onFinish = async () => {
    const payload = {
      ...form.getFieldsValue(),
    };
    const res = await _unitOfWork.spareSubCategory.update(
      spareSubCategory._id,
      payload
    );
    if (res) {
      form.resetFields();
  ...
  return (
    ...
    >
      ...
      >
        <Card title={t("spareSubCategory.update.title")}>
          <Row>
            <Col span={24}>
              <Form.Item
                name="spareCategory"
            ...
            <div className="modal-footer">
              ...
              <Button
                ...
                type="primary"
                onClick={() =>
                  Confirm(t("spareSubCategory.update.confirm"), () =>
                    onFinish()
                  )
                }
              >
                <SaveOutlined />{" "}
                {t("spareSubCategory.form.buttons.submit_update")}
              </Button>
            </div>
          </Row>
        </Card>
