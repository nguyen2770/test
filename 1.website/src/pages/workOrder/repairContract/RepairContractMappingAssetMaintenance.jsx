...
export default function RepairContractMappingAssetMaintenance() {
  ...
  useEffect(() => {
    setHeaderTitle(t("repair_contract.linking_repair_contracts_to_assets"));
    fetchServiceserviceContractorUserMappingss();
  }, []); // eslint-disable-line

  const fetchServiceserviceContractorUserMappingss = async () => {
    let res =
      await _unitOfWork.repairContract.getRepairContractMappingAssetMaintenancesByRes(
        { repairContract: prams.id }
      );
    if (res && res.code === 1) {
      setCalibrationContractMappingAssetMaintenance(
        res?.repairContractMappingAssetMaintenances
      );
    }
  };
  ...
  const onClickDelete = async (_record) => {
    let res =
      await _unitOfWork.repairContract.deleteRepairContractMappingAssetMaintenancesById(
        _record?.id
      );
    if (res && res.code === 1) {
      fetchServiceserviceContractorUserMappingss();
  ...
  const onSelectAssetMaintenance = async (_data) => {
    ...
    let res =
      await _unitOfWork.repairContract.createRepairContractMappingAssetMaintenance(
        data
      );
    if (res && res.code === 1) {
