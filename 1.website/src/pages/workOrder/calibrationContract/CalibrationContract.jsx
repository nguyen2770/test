...
export default function CalibrationContract() {
  ...
  useEffect(() => {
    setHeaderTitle(t("menu.maintenance_request.calibration_contract"));
  }, [t]);

  useEffect(() => {
    if (page > 1) {
      fetchGetCalibrationContracts();
    } else {
      fetchGetCalibrationContracts(1);
    }
  }, [page]);

  const fetchGetCalibrationContracts = async (_page) => {
    let payload = {
      page: _page || page,
      limit: PAGINATION.limit,
      ...searchForm.getFieldsValue(),
    };
    let res = await _unitOfWork.calibrationContract.getCalibrationContracts(
      payload
    );
    if (res && res.code === 1) {
      setAmcs(res?.resultWithItems);
      setTotalRecord(res?.totalResults);
    }
  };
  const onClickCreate = () => {
    navigate(staticPath.createCalibrationContract);
  };
  const onUpdate = (value) => {
    navigate(staticPath.updateCalibrationContract + "/" + value.id);
  };
  const onDetail = (value) => {
    navigate(staticPath.viewCalibrationContract + "/" + value.id);
  };
  const onMapping = (value) => {
    navigate(staticPath.calibrationMappingAssetMaintenance + "/" + value.id);
  };
  const onDelete = async (value) => {
    let res = await _unitOfWork.calibrationContract.deleteCalibrationContract(
      value?._id || value?.id
    );
    if (res && res.code === 1) {
      ShowSuccess("topRight", t("common.notifications"), t("common.messages.delete"));
      fetchGetCalibrationContracts();
    }
  };
    ...
    <Card>
      ...
      >
        <Row className="mb-1" gutter={32}>
          <Col span={6}>
            <Form.Item id="" label={t("orderPurchase.form.fields.contract_number")} name="contractNo">
