...
const PreventiveComment = () => {
  ...
  const fetchGetPreventiveComment = async () => {
    let payload = {
      page: page,
      limit: PAGINATION.limit,
      preventive: params.id,
    };
    let res = await _unitOfWork.preventive.getPreventiveComments(payload);
    if (res && res.code === 1) {
  ...
  const handleSend = async () => {
    ...
    const res = await _unitOfWork.preventive.createPreventiveComment(payload);
    if (res && res.code === 1) {
      setInput("");
      fetchGetPreventiveComment();
    }
  ...
  return (
    ...
    >
      <div style={{ flex: 1, overflowY: "auto", marginBottom: 16, display: "flex", flexDirection: "column-reverse" }}>
        {comments &&
      ...
      <div style={{ borderTop: "1px solid #eee", paddingTop: 12 }}>
        ...
        >
          ...
          <Button onClick={() => navigate(-1)}>{t("preventive.buttons.cancel")}</Button>
          <Button
            type="primary"
            icon={<SendOutlined />}
            onClick={handleSend}
...
export default PreventiveComment;