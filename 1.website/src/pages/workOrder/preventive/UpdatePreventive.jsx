...
export default function UpdatePreventive() {
  ...
  useEffect(() => {
    ...
    if (assetMaintenceChange && assetMaintenceChange.assetModel) {
  ...
  const fetchGetPreventive = async () => {
    ...
    if (res && res.code === 1) {
      ...
      form.setFieldsValue({
        ...
        calendarEndBy: res.data?.calendarEndBy
          ? dayjs(res.data?.calendarEndBy)
          : null,
        scheduleDate: res.data?.scheduleDate
          ? dayjs(res.data?.scheduleDate)
          : null,
        serial: data?.serial,
        preventiveName: isClone ? null : res.data.preventiveName,

      });
      setPreventive(res.data);
      setConditionBasedMaintenanceSchedules(
        res?.data?.preventiveConditionBaseds || []
      );
  ...
  const onFinish = async () => {
    ...
    if (isClone) {
      let payload = {
        preventive: {
          ...values,
          assetMaintenance: assetMaintenceChange._id,
        },
        preventiveTasks: serviceTasks,
        preventiveSpareParts: listSparePart,
        preventiveConditionBaseds: conditionBasedMaintenanceSchedules,
      ...
      const res = await _unitOfWork.preventive.updatePreventive(payload);
      if (res && res.code === 1) {
        navigate(-1);
        ShowSuccess(
          "topRight",
        ...
        extra={
          <>
            ...
            <Button type="primary" htmlType="submit">
              <PlusCircleFilled />
              {isClone ? t("preventive.buttons.create") : t("preventive.buttons.update")}
            </Button>
          </>
        }
      >
        <Row gutter={32}>
          ...
          <Row gutter={32}>
            {scheduleBasedOn === ScheduleBasedOnType.Calendar && (
              <Calendar form={form} preventive={preventive} />
            )}
