...
export default function CreatepurchaseOrder() {
    ...
    const handleAddDetail = async (newData) => {
        if (newData.id) {
            newData = { ...newData, receiptIssue: newData.id }
        }
    ...
    const onFinish = async () => {
        ...
        const payload = {
            ...
            stockIssueDetail: [...data],
        }

        const res = await _unitOfWork.receiptIssue.createReceiptIssue(payload);
        if (res) {
            message.success(t("stockIssue.messages.createSuccess"))
            navigate(-1)
        }
    };
    ...
    return (
        <div>
            <Form
                ...
                onFinish={() => Confirm(t("stockIssue.confirm.create"), () => onFinish())}
            >
                <Card
                    title={t("stockIssue.titles.create")}
                    extra={
                        <>
