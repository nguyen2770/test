...
export default function RequestIssue() {
    ...
    useEffect(() => {
        setHeaderTitle(t("stockIssue.header.liquidation"));
        // fetchDepartments();
        // fetchBranches();
    }, []);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            console.error("Failed to delete receipt issue:", error);
            message.error(t("stockIssue.messages.deleteError"));
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_text) => {
                ...
                const label = option ? t(option.label) : t(`stockIssue.exportType.${String(_text).toLowerCase()}`);
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
                    {
                        checkPermission(permissions,
                            permissionCodeConstant.stock_issue_liquidation_approve
                            ...
                            <Tooltip title={action === "pendingApproval" ? t("stockIssue.tooltips.approve") : t("stockIssue.tooltips.viewDetail")}>
                                <Button
                                    ...
                                    onClick={() => navigate(`${staticPath.liquidationWarehouseReceiptApproved}/${record.id}`)}
                                />
                            </Tooltip>
                        )
                    }
                    ...
                    {action === "pendingApproval" && (
                        <>
                            {
                                checkPermission(permissions,
                                    permissionCodeConstant.stock_issue_liquidation_edit
                                    ...
                                    <Tooltip title={t("stockIssue.actions.edit")}>
                                        <Button
                                            ...
                                            icon={<EditOutlined />}
                                            size="small"
                                            onClick={() => navigate(`${staticPath.liquidationWarehouseReceiptUpdate}/${record.id}`)}
                                        />
                                    </Tooltip>
                                )
                            }
                            ...
                            {
                                checkPermission(permissions,
                                    permissionCodeConstant.stock_issue_liquidation_delete
    ...
    const onClickGoToCreate = () => {
        navigate(staticPath.liquidationWarehouseReceiptCreate);
    };
            ...
            >
                <Row gutter={16} className="mb-1">
                    ...
                    <Col span={8} style={{ textAlign: "right" }}>
                        ...
                        {checkPermission(
                            permissions,
                            permissionCodeConstant.stock_issue_liquidation_create
                        ) && (
