...
import { FORMAT_DATE, PAGINATION, frequencyAllOptions, reportView, schedulePreventiveTaskAssignUserStatus } from '../../../utils/constant';
...
import PdfReportEngineerPerformanceInSchedulePreventive from './PdfReportEngineerPerformanceInSchedulPreventive';
...
const ReportEngineerPerformanceInSchedulePreventive = () => {
    ...
    const [schedulePreventiveTaskAsignUsers, setSchedulePreventiveTaskAsignUsers] = useState([]);
    ...
    useEffect(() => {
        setHeaderTitle(t("report.engineerSchedulePreventive.title"))
    }, []);
    ...
    const fetchGetSchedulePreventives = async (_page, searchValue) => {
        ...
        if (onChangeOption === reportView.summary) {
            ...
            if (res && res.code === 1) {
                setSchedulePreventiveTaskAsignUsers(res?.scheduleGroups);
                setTotalRecord(res?.totalResults)
                const _data = [];
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.new"),
                    value: res?.totalSchedulePreventiveTaskAssignUserNew || 0
                })
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.in_progress"),
                    value: res?.totalSchedulePreventiveTaskAssignUserInProgess || 0
                })
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.rejected"),
                    value: res?.totalSchedulePreventiveTaskAssignUserRejected || 0
                })
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.closed"),
                    value: res?.totalSchedulePreventiveTaskAssignUserClosed || 0
                })
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.total_jobs"),
                    value: res?.totalSchedulePreventiveTaskAssignUsers || 0
                })
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.total_usage_time"),
                    value: formatMillisToHHMMSS(res?.totalTimeConsumed)
                })
                setData(_data)
            }
        } else {
            ...
            if (res && res.code === 1) {
                setSchedulePreventiveTaskAsignUsers(res?.schedulePreventiveTaskAssignUsers);
                setTotalRecord(res?.totalResults)
                const _data = [];
                _data.push({
                    title: t("report.engineerSchedulePreventive.columns.new"),
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            const res = isSummary
                ? await _unitOfWork.reportSchedulePreventive.getSummaryReportEngineerPerformanceInSchedulePreventive(payload)
                : await _unitOfWork.reportSchedulePreventive.getDetailsReportEngineerPerformanceInSchedulePreventive(payload);
            const list = res?.scheduleGroups || res?.schedulePreventiveTaskAssignUsers;
    ...
    const prepareDataForExcel = (list, isSummary, t) => {
        return isSummary
            ? list.map(item => ({
                ...item,
                totalTimeConsumed: formatMillisToHHMMSS(item?.totalTimeConsumed),
            }))
            : list.map(item => {
                const frequencyLabel = t(parseToLabel(
                    frequencyAllOptions.Option,
                    item?.schedulePreventive?.preventive?.frequencyType
                ));
                const fullFrequency = item?.schedulePreventive?.preventive?.calenderFrequencyDuration
                    ? `${item?.schedulePreventive?.preventive?.calenderFrequencyDuration} ${frequencyLabel}`
                    : frequencyLabel;
                return {
                    ...item,
                    frequency: fullFrequency,
                    startDate: parseDate(item?.schedulePreventive?.startDate),
                    status: t(parseToLabel(schedulePreventiveTaskAssignUserStatus.Options, item?.status)),
                    totalTimeConsumed: formatMillisToHHMMSS(item?.totalTimeConsumed),
                    totalPlanningHours: formatMillisToHHMM(item?.totalPlanningHours),
                }
            });
    }
    const onClickPdfExport = async (values) => {
        try {
            const blob = await pdf(
                <PdfReportEngineerPerformanceInSchedulePreventive data={values} />
            ).toBlob();
    ...
    const columns = [
        ...
        {
            title: t("report.engineerSchedulePreventive.columns.user"),
            dataIndex: "user",
            key: "user",
        ...
        ...(onChangeOption === reportView.details
            ? [
                {
                    title: t("report.engineerSchedulePreventive.columns.code"),
                    dataIndex: "schedulePreventive",
                    key: "schedulePreventive",
                    excelKey: "schedulePreventive.code",
                ...
                {
                    title: t("report.engineerSchedulePreventive.columns.schedule_date"),
                    dataIndex: "schedulePreventive",
                    excelKey: "startDate",
                    align: 'center',
                    render: (text) => parseDate(text?.startDate)
                },
                {
                    title: t("report.engineerSchedulePreventive.columns.status"),
                    dataIndex: "status",
                    align: 'center',
                    render: (text) => t(parseToLabel(schedulePreventiveTaskAssignUserStatus.Options, text))
                },
                {
                    title: t("report.engineerSchedulePreventive.columns.time_used"),
                    dataIndex: "totalTimeConsumed",
                    align: 'center',
                    render: (text) => formatMillisToHHMMSS(text)
                },
                {
                    title: t("report.engineerSchedulePreventive.columns.planned_time"),
                    dataIndex: "totalPlanningHours",
                    align: 'center',
    ...
    const fieldsConfig = onChangeOption === reportView.summary
        ? [
            ...
            {
                name: "code",
                labelKey: "report.engineerSchedulePreventive.columns.code",
                placeholderKey: "report.engineerSchedulePreventive.columns.code",
                component: "Input",
            },
            {
                name: "status",
                ...
                component: "Select",
                options: "schedulePreventiveTaskAssignUserStatus"
            },
        ];
    return (
        <Card className='p-3'>
            ...
            >
                ...
                <Card title={t("report.engineerSchedulePreventive.summary_title")} style={{ borderRadius: 8 }}>
                ...
                <Row className='mt-4'>
                    <Col span={24}>
                        <Table
                            ...
                            columns={columns}
                            key="id"
                            dataSource={schedulePreventiveTaskAsignUsers}
                            bordered
                            pagination={false}
                        />
                        <Pagination
