...
export default function CreateSuppilersNeed() {
    ...
    const fetchMaterialsBySuppliesNeed = async () => {
        try {
            const res =
                await _unitOfWork.suppliesNeed.getSuppliesNeedDetailById({ id });
    ...
    const onFinish = async () => {
        ...
        const convertedDetails = data.map((item) => ({
            ...
            sparePart: item.sparePart
                ? item.sparePart?.id || item.sparePart
                : null,
        ...
        const suppilersNeed =
            await _unitOfWork.suppliesNeed.updateSuppliesNeed({
                SuppliesNeed: {
                    id: id,
                    payload,
                },
    ...
    const handleAddMaterial = (newData) => {
    ...
    return (
        <div>
            <Form
                ...
                onFinish={() =>
                    Confirm(t("suppliesNeed.messages.confirm_update"), () => onFinish())
                }
            >
                <Card
                    title={t("suppliesNeed.form.update_title")}
                    extra={
                        <>
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message: t(
                                            "suppliesNeed.form.validation.required_code"
                                        ),
                                    },
                                ]}
                            >
