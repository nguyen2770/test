...
export default function UpdateAssetMaintenance() {
  ...
  const isUpdateDepreciationChecked = Form.useWatch(
    "isUpdateDepreciation",
    form
  );
  useEffect(() => {
    setHeaderTitle(t("assetMaintenance.form.update_title"));
    fetchGetAllCustomer();
  ...
  // const getdepreciationTypeCode = () => {
  //   const found = depreciationTypes.find(
  //     (item) => item.id === depreciationType
  //   );
  //   if (found) {
  //     return found.code;
  //   }
  ...
  const fetGetAssetMaintenanceById = async () => {
    ...
    if (res && res.code === 1) {
      form.setFieldsValue({
        ...
        serial: isClone ? null : res.data.serial,
        assetNumber: isClone ? null : res.data.assetNumber,
      });
  ...
  const onFinish = async () => {
    ...
    if (fileList.length > 0) {
      if (fileList[0]?.originFileObj) {
        ...
        resourceId = fileList[0]?.supportDocumentId || null;
      }
    }
    const data = {
      ...values,
      resource: resourceId,
      asset: assetModelChange?.asset?.id,
      assetModel: assetModelChange,
      id: params.id,
    };

    // nếu là nhân bản thì gọi api thêm mới
    if (isClone) {
      const res = await _unitOfWork.assetMaintenance.createAssetMaintenance(
        data
      );
      if (res && res.code === 1) {
        ShowSuccess(
      ...
      const res = await _unitOfWork.assetMaintenance.updateAssetMaintenance(
        data
      );
      if (res && res.code === 1) {
        ShowSuccess(
          "topRight",
          t("common.notifications"),
          t("assetMaintenance.form.buttons.update")
        );
        navigate(-1);
      } else {
        ShowError(
          "topRight",
          t("common.notifications"),
          res?.message || t("assetMaintenance.form.buttons.update")
        );
      }
    }
  };
                ...
                <Col span={12}>
                  ...
                  >
                    <Search
                      ...
                      allowClear
                      enterButton={t("assetMaintenance.actions.create")}
                ...
                <Col span={12} className="p-0">
                  ...
                  {assetStyle === 1 && (
                    <>
                      <Col span={24}>
                        <Form.Item
                          label={t("Tính lại giá trị khấu hao")}
                          labelAlign="right"
                          name="isUpdateDepreciation"
                      ...
                      <Col span={24}>
                        ...
                        >
                          <Select
                            ...
                            disabled={!isUpdateDepreciationChecked}
                          ></Select>
                        </Form.Item>
                      </Col>
                      <ChosseDepreciationType
                        selectedDepreciationType={depreciationType}
                        isDisable={!isUpdateDepreciationChecked}
                      />
                    </>
                  )}
                </Col>
