    ...
    try {
      ...
      if (res?.data) {
        form.setFieldsValue({
          ...
          taxGroupId: res.data.taxGroupId,
          resourceId: res.data.resourceId,
          customer_gst_number: res.data.customer_gst_number,
        ...
        if (res.data.countryId) {
          fetchStateByCountry(res.data.countryId);
        }
        if (res.data.stateId) {
          fetchCityByState(res.data.stateId);
        }
      }
    } catch (error) {
      console.error("Error fetching customer details:", error);
      message.error("Không thể tải thông tin người dùng tài sản!");
    }
  };
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical">
        <Card title="Chỉnh sửa người dùng tài sản">
          {/* Group 1: Name */}
          <FormItem
            name="customerName"
            label="Customer Name*"
